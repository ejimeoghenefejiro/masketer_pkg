...
def master_data(
    ...
    # ---- create placeholders as you did
    ...
    return rdata, df0
# ----------------- Globals used by the original notebook flow -----------------
...
def corr_ana(R, sector, rdata):
    if sns is None:
        raise ImportError("seaborn is required for corr_ana(); install seaborn or masketer[notebook].")
    ...
    sns.heatmap(corr, mask=mask, cmap=cmap, vmax=2, center=0, vmin=-2,
    ...
    cmap = sns.diverging_palette(230, 20, s=75, l=50, sep=2, n=5, center='light', as_cmap=True)
    sns.heatmap(corr1, mask=mask, cmap=cmap, vmax=2, center=0, vmin=-2,
                square=False, linewidths=1, cbar_kws={"shrink": 1}, annot=False)
